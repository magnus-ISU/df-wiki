<!-- Mirrored from https://dwarffortresswiki.org/index.php/Material_science (revision 310900).
     Generated by crawl.py - edit the wiki, not this file. -->

# Material science

*[Source](https://dwarffortresswiki.org/index.php/Material_science) &middot; revision 310900 &middot; retrieved 2026-09-12 &middot; [wikitext](../../mirror/wikitext/Main/Material_science.wiki)*

---
<table>
<tbody><tr><td><div><a href="https://dwarffortresswiki.org/index.php/File:Announce_migrant_icon.png"><img alt="Announce migrant icon.png" height="32" src="https://dwarffortresswiki.org/images/5/5f/Announce_migrant_icon.png" width="32"/></a></div></td><td>This article was migrated from <a href="../DF2014/Material_science.md">DF2014:Material science</a> and may be inaccurate for the current version of DF (v53.16). See <a href="../Dwarf_Fortress_Wiki/Version_migration.md">this page</a> for more information.  </td></tr></tbody></table>

<table><tbody><tr><td><div><b>v53.16</b> · <a href="../DF2014/Material_science.md">v0.47.05</a></div>This article is about the current version of DF.<br/>Note that some content may still need to be updated.<br/>
...
[Materials](Material.md) have a number of properties representing real-world variables that describe how they respond to inputs. In particular, the game has a number of variables that describe what happens to a material when it's put under stress.
...
In the material raws, whenever you see 'yield', 'fracture', or '[strain at yield](Strain_at_yield.md)', that property is a stress-related quality.
...
The following explanations assumes real world physics sort of apply (since [Toady One](Toady_One.md) chose real world properties). The game doesn't use all of these properties yet, and may not be applying them according to real world physics.
...
Tensile: Force applied by pulling on something, like suspending one object via another. (e.g., if you suspend an [elf](Elf.md) from a metal pole, you are applying a tensile force to the pole).
...
The formulae below have been reverse-engineered [\[1\]](http://www.bay12forums.com/smf/index.php?topic=131995.0) [\[2\]](http://www.bay12forums.com/smf/index.php?topic=142372.0) and experimentally proven [\[3\]](http://www.bay12forums.com/smf/index.php?topic=116151.0) [\[4\]](http://www.bay12forums.com/smf/index.php?topic=141364) by several independent researchers. Below are the simplified results; for more details see links above.

#### Attack Types

Both creatures and items can have \[ATTACK\] tokens. A creature can execute any of its natural attacks plus any attacks of the items it holds. The attacks marked with \[EDGE\] flag deliver *edged* damage which is governed by \[SHEAR\_\*\] tokens; they can be further differentiated by attack contact area: generally concentrated strikes (area of 50 or less) are considered *stabbing* while wider areas correspond to *slashing* attacks. This distinction shall be emphasized later.

Every other attack is considered *blunt*. \[IMPACT\_\*\] tokens affect blunt combat. Most specialised blunt weapons have small contact area; edged weapons generally also have blunt attacks with larger area values; items or creatures without defined attacks get default blunt attack with area = (size)^(2/3). Under certain circumstances edged attack can be converted to blunt, but not contrariwise.

Wrestling moves are special: **breaking bones** uses \[BENDING\_\*\] values, **pinching** utilizes \[COMPRESSIVE\_\*\] properties, and **biting** can deal \[TENSILE\] or \[TORSION\] damage depending on whether the attack is edged. Those attacks generally ignore armor.
...
| Body part | Relative size (human) | [Kobold](Kobold.md) | [Elf](Elf.md)/[Goblin](Goblin.md) | [Human](Human.md) | [Troll](Troll.md) |
| --- | --- | --- | --- | --- | --- |
...
Armor size is calculated as underlying body part size **times** size multiplier, where size multiplier is calculated as coverage/100% **times** [Layer_Size](Armor_token.md#layer-size)/100 **times** (1+([UPSTEP+UBSTEP+LBSTEP](Armor.md#coverage))/4); "MAX" counts as "3" in the last sum.

| Item | Size multiplier | Body part | [Dwarf](Dwarf.md) | [Human](Human.md) | Extra body parts covered (humanoid) | Notes |
| --- | --- | --- | --- | --- | --- | --- |
...
M = Size \* Str \* Vel / (10 \* (( 10<sup>5</sup> + i_Size)/W ))<sup>\[[Verify](https://dwarffortresswiki.org/index.php/Talk:Material_science)\]</sup>,
...
There is also a hard velocity limit (10000) that might skew these calculations, but it's actually impossible to reach in unmodded game. (Well, okay, if you're a zombie adventurer with maxed out strength you *might* reach the limit using an adamantine whip -- but [why](Stupid_dwarf_trick.md)?)
...
Vanilla [bolts](Bolt.md) and [arrows](Arrow.md) end up with a momentum of 50 (velocity nearly 20, at density 1667), as long as their density exceeds 1666. [Divine](Divine_metal.md) ammo (1.5kg) has momentum 30 (velocity 20), [bone](Bone.md) and most [wood](Wood.md) (0.75kg) get 15 (velocity 20), and [adamantine](Adamantine.md) bolts (0.3kg) have only 6 (velocity 20). Wooden [darts](Dart.md) (0.1kg) usually have 5 (velocity 50).
...
- **S** is weapon material [sharpness](Edge.md) multiplier (1x for most metals, 1.2x for [divine metal](Divine_metal.md), 1.5x for [glass](Glass.md), 2x for [obsidian](Obsidian.md), 10x for [adamantine](Adamantine.md) and 0.1x for all other materials)
- **Qw** is [quality](Quality.md) sharpness multiplier (0.5x for normal quality, 0.7x for fine, 1x for masterwork (or artifact) etc.)
- **Qa** is armor [quality](Quality.md) multiplier (same but x3 for artifacts)
...
Clothing with \[STRUCTURAL_ELASTICITY\_\*\] tokens has its stress properties modified.
...
- When dealing with dwarf-sized targets, layer **contact areas** usually lay in 200~10000 range. The majority of vanilla weapons, however, has contact areas either below or above that ([dagger](Dagger.md) is the lone exception); it therefore can be said, as a rule of thumb, that weapons with area of five or six digits assume their target's contact area, whereas the others use their own.
- Weapon **weight** matters very little past a certain threshold: for example, a platinum [war hammer](War_hammer.md) in dwarven hands only gets about 12% more momentum over a steel one, despite being thrice as heavy. (An adamantine hammer, however, only has 1/7th as much.) Thus, since all common weapon metals have about the same density, it can be safely ignored.
  - The only exception are *weapon traps*, which are much more effective with heavy weapons loaded.
- **Shear yield** doesn't actually matter. Even with [dagger](Dagger.md)/[bolt](Bolt.md)'s contact area of 5 it contributes only ~15% to piercing cost, and since it equals about half of shear fracture for most metals, it can be approximated as such without much error.
...
- Chain mail cannot block attacks via momentum cost thresholds; it still can blunt slashing attacks and then deflect them. Thus, the best defence can be reached by wearing *dense* (like [copper](Copper.md)) mail shirt under a *rigid* (like [candy](Candy.md)) one.
...
With that in mind, here are some numbers for vanilla weapon/armor materials:<sup>\[[Verify](https://dwarffortresswiki.org/index.php/Talk:Material_science)\]</sup>
...
**Categories:** [Migrated pages needing review](../Category/Migrated_pages_needing_review.md) &middot; [Unknown Quality Articles](../Category/Unknown_Quality_Articles.md) &middot; [Current](../Category/Current.md) &middot; [Material properties](../Category/Material_properties.md) &middot; [Verify](../Category/Verify.md) &middot; [Materials](../Category/Materials.md)

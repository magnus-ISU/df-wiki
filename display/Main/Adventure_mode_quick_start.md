<!-- Mirrored from https://dwarffortresswiki.org/index.php/Adventure_mode_quick_start (revision 309887).
     Generated by crawl.py - edit the wiki, not this file. -->

# Adventure mode quick start

*[Source](https://dwarffortresswiki.org/index.php/Adventure_mode_quick_start) &middot; revision 309887 &middot; retrieved 2026-09-11 &middot; [wikitext](../../mirror/wikitext/Main/Adventure_mode_quick_start.wiki)*

---
<table>
<tbody><tr><td><div><a href="https://dwarffortresswiki.org/index.php/File:Announce_migrant_icon.png"><img alt="Announce migrant icon.png" height="32" src="https://dwarffortresswiki.org/images/5/5f/Announce_migrant_icon.png" width="32"/></a></div></td><td>This article was migrated from <a href="../DF2014/Adventure_mode_quick_start.md">DF2014:Adventure mode quick start</a> and may be inaccurate for the current version of DF (v53.16). See <a href="../Dwarf_Fortress_Wiki/Version_migration.md">this page</a> for more information.  </td></tr></tbody></table>
...
<table><tbody><tr><td><div><b>v53.16</b> · <a href="../DF2014/Adventure_mode_quick_start.md">v0.47.05</a></div>This article is about the current version of DF.<br/>Note that some content may still need to be updated.<br/>
<div><a href="../Dwarf_Fortress_Wiki/V.md">More Info</a> · <a href="../Template/Av.md">V</a></div></td></tr>
</tbody></table>

> *See [Adventure Mode quick reference](Adventure_Mode_quick_reference~2.md) to quickly look up key commands. If you're a more experienced player (perhaps directed here in error) look up the [Tutorial:Powerplay Guide](../Tutorial/Powerplay_Guide.md) for instruction, instead.*

This guide provides step-by-step instruction for those who have never played *Dwarf Fortress* in **Adventure Mode** before. It assumes that you have already installed the game and can navigate the menus on your own, but no significant knowledge of how to play adventure mode is required - see the full [adventure mode](Adventure_mode~2.md) documentation for additional details.

### Common UI concepts

**Key Reference**

Most of the key commands you will need are noted on this page, but you can refer to the in-game help with ? or the [quick reference](Adventure_Mode_quick_reference~2.md) page to look up the key for a particular action.
...
Some custom worlds which are interesting for fortress mode, may be annoying for adventure mode for reasons such as: few civilizations, low population, races not existing, lack of metals, or history being so far advanced that many fortresses and [towns](Town.md) have been abandoned.
...
- With a less powerful computer, you may want to change world size to small instead of medium, both to reduce generation time and increase [FPS](FPS.md).
- A short history will limit the chance of some civilization-destroying cataclysm, and finish generating sooner.
  - However, this will also reduce the number of "developed" religions with [temples](Temples.md), limiting your options for playing as a [chosen](Chosen.md). Since playing as [chosen](Chosen.md) provides more direction in gameplay and helpful buffs, it's a helpful option for new players.
...
After choosing Start Playing and Adventurer, the game will simulate two weeks of game time in the world. After that, the first steps in starting an [adventure mode](Adventure_mode~2.md) game are to choose the [race](Race.md) and status of your adventurer, then their [civilization](Civilization.md). All major races can complete the same quests, though larger creatures generally do better in combat. What you really want, though, is to start as a member of a [dwarven](Dwarven.md) civilization, or at least a [human](Human.md) one; this will give you access to the best equipment.

The races available to play as adventurers will vary by world. Status mostly determines the number of points available in character creation, with demigod giving the most. Civilizations can have access to different materials, and while they start with only one race as members, other races can join them throughout world simulation; that can include the other main races, [animal people](Animal_people.md), etc. The main race of a civilization isn't shown on the screen where you choose one, but can be determined by most of the [site symbols](Site.md#site-types) shown on the map; dwarven are mostly Ω, human are mostly #, and [elven](Elven.md) are mostly î. Also, on the next character creation page, their sites will be described as *dwarven*, *human*, or *elven*.
...
Your [name](Name.md) has little bearing on gameplay, so that can be freely selected with e, c, and r. If you customize your first name, people may comment on how unusual it is. If you define something in every category at the start, then your name won't grow with [fame](Name.md#epithets), though it can change with infamy. Similarly; [gender](Gender.md) rarely makes a difference in gameplay, and can be changed with g – for most playable races, the only differences are the presence of [beards](Beard.md) and [geldables](Gelder.md). A few [animal people](Animal_people.md) races have more significant differences, such as [moose](Moose.md) people, where males are up to 2/3 larger and have a gore attack.

Your home determines where you start, as well as the available occupations and beliefs. Dwarven fortresses, [mountain halls](Mountain_halls.md), and some sites taken over from other civilizations can be hard to navigate out of, and sites with many inhabitants can negatively affect [framerate](Framerate.md).

Occupation determines some free starting [skills](Skill.md) and possibly a starting weapon; [hunter](Hunter.md) and [ranger](Ranger.md), for example, add some skills that are useful in combat. A [hearthperson](Hearthperson.md) starts as a member of the local military and can receive orders from their superior, if the latter can be found. Many other occupations cannot currently be practiced in adventurer mode.

Beliefs determine if you worship a [deity](Deity.md), and are a member of any [religion](Religion.md). Being more than a casual worshiper of a deity can add the need to pray, which can easily be done by [talking](Talking.md) to them.
...
See the [Attribute](Attribute.md) page for descriptions of each; [Skills by Body Attribute](Attribute.md#skills-by-body-attribute) and [Skills by Soul Attribute](Attribute.md#skills-by-soul-attribute) also show how each attribute is applied. Reducing attributes below the default only gives 1 point; so you would have to lower many to raise one.
...
Here, you can modify the equipment your adventurer starts with, and add [mounts](Mount.md) or [pets](Pet.md); they are different pages, but use the same pool of points. The initial equipment automatically added by the game will vary depending on the civilization, occupation, and skills. New items are added with n, the list of possible items can be filtered by typing the name; only items not already added are listed. Using + increases the quantity of the selected item, while - decreases it, or removes the last item. Using w & q, you can change the [quality](Quality.md) of items; w on the backpack, which makes it a -backpack-, can be useful for satisfying the need to be extravagant, if you added it earlier.

Some animals can be used as mounts, some as pack animals, and some as both; unfortunately, the game does not indicate which, so you will have to rely on guessing, or the animal's page here on the wiki. Another way to get mounts and pack animals, is that stray animals encountered in the game can be adopted. Animals that are classified as [vermin](Vermin.md) will be carried as inventory; others will be separate creatures that try to follow you around.

Waterskins will start filled with water. Some items will already be in containers, other items will start in the hands; the latter includes alcohol, so the first thing to do in-game is to put those in containers with p. Any points not used will be converted to [coins](Coins.md), so if you have many points remaining, you may want to spend them on gems, as they are much lighter, and better for trading. Adventurers may still start with a few coins, even if all points were spent.
...
[![](https://dwarffortresswiki.org/images/thumb/2/28/Advmode_conversation_v50.png/400px-Advmode_conversation_v50.png)](https://dwarffortresswiki.org/index.php/File:Advmode_conversation_v50.png)

[https://dwarffortresswiki.org/index.php/File:Advmode_conversation_v50.png](https://dwarffortresswiki.org/index.php/File:Advmode_conversation_v50.png)
...
- **Beasts** - any kind of creature that may be harassing people, which can range from [vampires](Vampire.md) living amongst the inhabitants of a site, all the way up to [titans](Titan.md) and other mythical creatures. Avoid the latter at first - a [hydra](Hydra.md), for example, presents a massive challenge even to the most experienced adventurer.

- **[Bandits](Bandit.md)** - groups of people who menace people in nearby sites, out of the reach of civilization. They usually hang out in [camps](Camp.md) (shown by a `☼` on the map). Killing bandits is a great way to raise your reputation, as nobody cares what happens to them.

- **Criminals** are similar to bandits, except they operate right under the nose of authorities -- sites ruled by civilizations. You will find these outcasts hiding in abandoned [houses](House.md) and underground cells. Unlike bandits, criminals are often considered productive members of society. If you start a fight with some on a whim, you may find that you're now an enemy of a criminal empire comparable to (or more powerful than) your home civilization.

- **Skulking Vermin** are thieving [kobolds](Kobold.md), and otherwise similar to bandits. Due to their relative ineptitude in combat, these are probably the best targets for fledgling adventurers.
...
[![](https://dwarffortresswiki.org/images/thumb/9/9a/Fast_travel_preview.png/400px-Fast_travel_preview.png)](https://dwarffortresswiki.org/index.php/File:Fast_travel_preview.png)

[https://dwarffortresswiki.org/index.php/File:Fast_travel_preview.png](https://dwarffortresswiki.org/index.php/File:Fast_travel_preview.png)
...
- [Meat](Meat.md) - probably the easiest to come by - find an [animal](Animal.md) in the wilds, kill it and butcher the [corpse](Corpse.md) with a sharp tool. Some animals are too small to butcher and cannot be eaten.
- [Fruits](Fruit.md) - they grow on the ground and on [trees](Tree.md) during the [summer](Calendar.md). Just ensure the season is appropriate - you won't find any fruits in the winter!
- Certain [leaf](Leaf.md)-bearing plants are edible, and grow year-round.
- Exploring human and dwarf [sites](Site.md) - there are [bags](Bag.md) of food stored in the houses. Some Shaping Trees in the elven [forest retreats](Forest_retreat.md) may have fruits and vegetables in or out of season, or you can buy some in city-sized human, dwarven and elven settlements.

and if you search for something to quench your thirst:

- Drink fresh [water](Water.md) directly from a [river](River.md), or fill your waterskin (or any other container; a [backpack](Backpack.md), for example, can hold 50 units of water, though a quiver stores a more reasonable \[i.e., lighter\] amount) with it.
- [Hamlets](Hamlet.md) and cities can have [wells](Well.md) if they have developed enough. The building interact key (default u) can draw a bucket of water (or ice) from it, which can be drunk or filled from.
- [Snow](Snow.md) can be found in cold biomes. Pick some from the ground and melt it near a [campfire](Campfire.md).
- You can drink [blood](Blood.md), even when you aren't a [vampire](Vampire.md). Wound a creature to make it bleed and then drink some blood from the pool that forms.
- If you prefer to drink [alcohol](Alcohol.md), search for [barrels](Barrel.md) of [booze](Booze.md) inside dwarven [fortresses](Fortress.md), or order a drink at a [tavern](Tavern.md).
...
For your first fight, you may want to let your companions take the lead anyway, though, as you may not have any decent armor yet. Press . or , to skip instants, if you want to wait in one spot and let your friends advance and soften up some enemies first. If you want to be useful while sitting back and letting your friends charge in first, use t to throw objects such as rocks (freely available on outdoor [pebbles](Pebbles.md) tiles by pressing g) at your opponent.

<br> If one of your companions dies, no big deal - you can just take their stuff by pressing g, then get another follower later. If they die and you're outnumbered, just run away and come back later with new companions.
...
If you must choose which items to keep, try to keep the ones of the best material and [item quality](Item_quality.md).
...
You can tell at a glance which [towns](Town.md) have shops by looking for white roads in towns (rather than the yellow roads that are houses most of the time) on the travel map.
...
You may want to read through the full [adventure mode](Adventure_mode~2.md) documentation to learn about things that weren't covered here.

### Feedback

[Thread about this tutorial on the Bay12 Forums](http://www.bay12forums.com/smf/index.php?topic=80637.0)

**Getting Started**

[About](About.md) - [Installation](Installation.md) - [Tutorials](Tutorials.md) - [FAQ](Frequently_Asked_Questions.md)

[Fortress Mode Quickstart Guide](Quickstart_guide.md) - Adventure Mode Quickstart Guide
...
**Categories:** [Migrated pages needing review](../Category/Migrated_pages_needing_review.md) &middot; [Needs update](../Category/Needs_update.md) &middot; [Tattered Quality Articles](../Category/Tattered_Quality_Articles.md) &middot; [Current](../Category/Current.md) &middot; [Getting started](../Category/Getting_started.md) &middot; [Adventurer mode](../Category/Adventurer_mode.md)

<!-- Mirrored from https://dwarffortresswiki.org/index.php/DF2014:Modding (revision 272553).
     Generated by crawl.py - edit the wiki, not this file. -->

# DF2014:Modding

*[Source](https://dwarffortresswiki.org/index.php/DF2014:Modding) &middot; revision 272553 &middot; retrieved 2026-09-13 &middot; [wikitext](../../mirror/wikitext/DF2014/Modding.wiki)*

---
<table><tbody><tr><td><div><a href="../Main/Modding.md">v53.16</a> · <b>v0.47.05</b> · <a href="https://dwarffortresswiki.org/index.php/v0.34:Modding">v0.34</a> · <a href="https://dwarffortresswiki.org/index.php/v0.31:Modding">v0.31</a> · <a href="https://dwarffortresswiki.org/index.php/40d:Modding">v0.28</a> · <a href="https://dwarffortresswiki.org/index.php?title=23a:Modding&amp;action=edit&amp;redlink=1">v0.23</a></div>This article is about an <b>older version</b> of DF.<br/>
...
- [Official bay12 Forum for modding](http://www.bay12forums.com/smf/index.php?board=13.0)
- [Utilities#Modding tools](Utilities.md#modding-tools)
- [Character table](Character_table.md)

Guides:

- [Guide to Modding](#guide)
- [Memory hacking](Memory_hacking.md), [Offset Finding Methods](../Main/Offset_Finding_Methods.md)

Game Files:

- Gameplay settings: [announcements.txt](Announcements.txt.md) • [d_init.txt](D_init.txt.md)
- Settings: [colors.txt](Colors.txt.md) • [init.txt](Init.txt.md) • [interface.txt](Interface.txt.md)
- Log files: [errorlog.txt](Errorlog.txt.md) • [gamelog.txt](Gamelog.txt.md)
- [Saved game folder](Saved_game_folder.md)

Miscellaneous

- Development: [Release information](Release_information.md) • [Developer diaries](../Main/Dwarf_Fortress_Talk.md) • [Roadmap](../Main/Dwarf_Fortress_Development.md)
- [Screenshot](../Main/Screenshot.md)
...
All the base data that can be edited by prospective modders can be found in the \\raw\\ folder. This folder contains two subfolders: "graphics" (where you insert [graphics sets](Graphics_set_repository.md)), and "objects", which contains all the data for, generally, everything in the game that is not hardcoded.

Within the \\raw\\objects folder are a large number of text files - these are the [raw files](Raw_file.md), and editing them is quite easy - you can also create your own if you wish. For now, take a look at one of the existing files. For example, if you open creature_standard.txt, it should look something like this:
...
You can find many details about the rest of the civilization tokens [here](Entity_token.md).
...
- [\[ACTIVE_SEASON\]](Entity_token.md#active-season) - Defines the seasons when an entity may visit your fortress.
- [\[PROGRESS_TRIGGER\_\*\]](Entity_token.md#progress-trigger-population) - Defines the triggers which control when an entity will become interested in your fortress.
- [\[COMMON_DOMESTIC_PACK\]](Entity_token.md#common-domestic-pack) - Allows the civilization to use domestic pack animals. If an entity lacks pack animals (pr ability to pull wagons), it will be unable to send caravans (showing as No Trade at the [embark](Embark.md) screen), unless it has domesticated any suitable animal species.
- [\[COMMON_DOMESTIC_PULL\]](Entity_token.md#common-domestic-pull) - Allows the civilization to use domestic animals to pull [wagons](Wagon.md), assuming their [KILL_PLANT ethic](Ethic.md#kill-plant) permits them to use wagons in the first place.
- [\[MERCHANT_BODYGUARDS\]](Entity_token.md#merchant-bodyguards) - Caravan will be guarded by [soldiers](Soldier.md).

#### Modding creatures

Creature modding is great fun – you can change nearly any aspect of a creature, or make your own completely from scratch.

Modding creatures is very similar to modding civs: it's just a matter of editing, adding, or removing tokens, enclosed in square brackets underneath the creature's \[CREATURE:\] header. The creature entries contain all of the information about each and every non-random creature in the game, from animals to dwarves to goblins to even caravan wagons. A lot of the creature tokens are fairly self-explanatory; you can find a list of such tokens [here](Creature_token.md). But before you start creating your own creatures, you'll want to learn how the tissues system works.
...
Note that this makes use of DF's built-in temperature scale. You can read more about that [on this page](Temperature.md). We're also referencing material tokens, which we haven't gone over yet - we'll talk about making your own materials later.
...
Plants are, again, not unlike creatures. With what you've learned so far in regard to tokens and the materials system, running through the notes included in plant_standard.txt should explain most things. [Here's the list of plant-specific tokens](Plant_token.md).
...
Let's look at this line by line:<br> First, we define its file name. In this case it's MUSHROOM_HELMET_PLUMP. Next we define its in-game name (plump helmet) and its adjective for if you were to craft with it (e.g. plump helmet earrings).
...
Next, \[PICKED_TILE:161\] is the character (161 in this case) shown when the crop is harvested. See [character table](../Main/Character_table~2.md) for a table of usable tiles. \[PICKED_COLOR:6:13:0\] is the color used for the crop's tile when harvested. It's in a foreground:background:brightness format. See [color](Color.md) for the colors usable.

```
 	[PICKED_TILE:161][PICKED_COLOR:6:13:0]
```

\[GROWDUR:300\] is how long it takes for your crop to grow. There are 1008 growdur units in a season.<br> \[VALUE:2\] Is the value of the harvested plant (default 1).
...
Adding the token [DISPLAY_COLOR](Material_definition_token.md#display-color) directly after \[USE_MATERIAL_TEMPLATE\] would allow us to change the color of the tree.
...
would give us a dark blue apple tree. This method is used by the game in [birches](Birch.md) and [various](Spore_tree.md) [underground](Nether-cap.md) [trees](Blood_thorn.md).
...
These are the name of the workshop ("Soap Maker's Workshop") and [color](Color.md) of the workshop's name when examined with 'q' (White with a black background).
...
These refer to items required to build the building. These are in the same format as [reaction reagents and products](Reaction.md)--quantity:[item](Item_token.md):[material](Material_token.md). You'll learn more about those on the article about [reactions](Reaction.md), though. The second BUILD_ITEM is special-- it uses modifiers exclusively to determine its requirements. BUILDMAT refers to wood logs, wood blocks, stone boulders, and stone blocks; WORTHLESS_STONE_ONLY means it can't use economic stone; CAN_USE_ARTIFACT means that it... can use artifacts. EMPTY, in the bucket's case, means that the bucket must be empty.

More can be seen at the [building tokens](Building_token.md) article.

#### Reactions

Reactions are the crafting recipes used in [workshops](Workshop.md), and by the [adventurer](Adventurer_mode.md). By adding new reactions you can make new items available, or enable you to get items or materials in new ways. The reactions can also be given to entities, in which case they will make use of them during both world gen and play; making a reaction that creates [steel](Steel.md) directly from [plant fibers](Plant_fiber.md), would allow the elves to craft steel and arrive clad in it in a [siege](Siege.md).

Not all crafting reactions are defined in the raws, but some are, such as those for [pottery](Ceramic_industry.md) and [alloy](Metal.md#alloys-2) making.
...
> *Main articles: [Category:DF2014:Modding_Examples](../Category/DF2014%253AModding_Examples.md)*
...
> *Main article: [Utilities § Modding_tools](Utilities.md#modding-tools)*

There are several utilities that assist in modding efforts. There is [a list of them](http://www.bay12forums.com/smf/index.php?topic=28829.0) on the [Bay 12 Forums](Bay_12_Forums.md).

### See also

<table><tbody><tr><th>
<div>
<div><a href="../Category/DF2014%253AModding.md">View the<br/>related<br/>category</a></div>
<div><a href="../Category/DF2014%253AModding.md"><img alt="Catback.png" height="94" src="https://dwarffortresswiki.org/images/5/50/Catback.png" width="84"/></a></div>
</div>
</th></tr></tbody></table>
...
**Categories:** [DF2014:Exceptional Quality Articles](../Category/DF2014%253AExceptional_Quality_Articles.md) &middot; [DF2014:Modding Examples](../Category/DF2014%253AModding_Examples.md) &middot; [DF2014:Modding](../Category/DF2014%253AModding.md) &middot; [DF2014:Guides](../Category/DF2014%253AGuides.md)

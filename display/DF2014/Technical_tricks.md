<!-- Mirrored from https://dwarffortresswiki.org/index.php/DF2014:Technical_tricks (revision 274827).
     Generated by crawl.py - edit the wiki, not this file. -->

# DF2014:Technical tricks

*[Source](https://dwarffortresswiki.org/index.php/DF2014:Technical_tricks) &middot; revision 274827 &middot; retrieved 2026-09-13 &middot; [wikitext](../../mirror/wikitext/DF2014/Technical_tricks.wiki)*

---
<table><tbody><tr><td><div><a href="../Main/Technical_tricks.md">v53.16</a> · <b>v0.47.05</b> · <a href="https://dwarffortresswiki.org/index.php/v0.34:Technical_tricks">v0.34</a> · <a href="https://dwarffortresswiki.org/index.php/v0.31:Technical_tricks">v0.31</a> · <a href="https://dwarffortresswiki.org/index.php/40d:Technical_tricks">v0.28</a> · <a href="https://dwarffortresswiki.org/index.php/23a:Technical_tricks">v0.23</a></div>This article is about an <b>older version</b> of DF.<br/>
<div><a href="../Dwarf_Fortress_Wiki/V.md">More Info</a> · <a href="../Template/Av.md">V</a></div></td></tr>
</tbody></table>

In order to configure *Dwarf Fortress* to your liking there are two files you can edit, [init.txt](Init.txt.md) and [d_init.txt](D_init.txt.md). The first file, init.txt, contains mostly the settings pertaining to the window, rendering and sound of *Dwarf Fortress*. The second file, d_init.txt, contains settings that affect game features such as saving and population controls.
...
Change the argument from "YES" to "NO" to turn off the [intro movie](Intro_movie.md).
...
\[WINDOWEDX:98\]\[WINDOWEDY:98\] will make window big enough to display whole 2x2 [embark](Embark.md) [site](Site.md) with 8x8 [tileset](Tileset.md) or one embark tile with 16x16 tileset.
...
Can be "YES" or "NO". If this is "YES", *Dwarf Fortress* will use the raw/graphics folder for certain tile graphics. Currently this is limited to [creature graphics](Graphics_set_repository.md).
...
The [font](../Main/Tileset_repository~2.md) used by *Dwarf Fortress* when in windowed mode and \[GRAPHICS:YES/NO\] is "YES".
...
Turns [temperature](Temperature.md) *calculations* on or off. If temperature calculations are off, only a few direct temperature changes will take place. For example, [magma](Magma.md) will set tiles and creatures to high temperatures, but those tiles will never cool and those creatures will not catch fire. Effects that rely on temperature calculations, such as water freezing, melting, or evaporating, or creatures and items taking temperature-related damage, will not occur.

- \[WEATHER:YES\]

Turns [weather](Weather.md) on or off. If weather is off it will never rain or snow.
...
Turns on or off fortress mode [sieges](Siege.md). Turning this off for your first couple of games may make the game easier to start with.

- \[CAVEINS:YES\]

If this is "YES" it is possible for [cave-ins](Cave-in.md) to occur.

- \[ARTIFACTS:YES\]

Turns on or off [strange moods](Strange_mood.md) and the resulting [artifacts](Artifact.md).
...
Turns on or off the spread of blood spatter and other contaminants between creatures and ground tiles in [Fortress mode](Fortress_mode.md). As contaminants are [buggy](Blood.md) and can have a major negative impact on [FPS](FPS.md) in [Fortress mode](Fortress_mode.md), it is recommended to leave this set at "NO" for now.

- \[WALKING_SPREADS_SPATTER_ADV:YES\]

Turns on or off the spread of blood spatter and other contaminants between creatures and ground tiles in [Adventurer mode](Adventurer_mode.md).

- \[PATH_COST:1:2:5:25\]

The pathfinding costs associated with different [traffic](Traffic.md) values.

- \[COFFIN_NO_PETS_DEFAULT:NO\]

When building a [burial receptacle](Coffin.md), the option to allow pets to be buried in it will default to NO if this is set to "YES".
...
If "YES" *Dwarf Fortress* will display the [quality](Quality.md) of an item's improvements in the name. If you get annoyed by seeing items like \*\<\*sword\*\>\* you can get rid of the outside \*\* by setting this to NO.
...
If this value is YES, then it will always pop up a warning screen when you embark (Like you get when you have [Salt Water](https://dwarffortresswiki.org/index.php/v0.34:Water#Salt_Water) or an [Aquifer](https://dwarffortresswiki.org/index.php/v0.34:Aquifer) on-site)
...
The tile displayed for pillars. Pillars are created at the ends of strings of wall tiles. Some [tilesets](Tilesets.md) may have been designed to use tile 9, 10, or 255 for this, even some that are primarily ASCII, and may or may not include a custom d_init.txt file to do so.

- Multiple \[TRACK\_\*\] and \[TREE\_\*\] lines

The tiles displayed for most of the various possible [track](Track.md) or [tree](Tree.md) parts. Not all the track tiles can be customized, such as the [rollers](Roller.md) or the track stop. By default, the track tiles are followed by an I to have them displayed with inverted colors so that they don't look the same as walls. Some tilesets may have been designed to use tile 9, 10, or 255 for tree trunk pillars, instead of the default 'O'. Note that the TREE_TRUNK\_\* and TREE_CAP_WALL\_\* corner tokens use a different naming convention than the other tokens; For example, by default TREE_TRUNK_NW uses tile 201 which is ╔, while TREE_BRANCH_NW uses 217 which is ┘, and TRACK_NW uses 188 (with inverted colors) which is ╝.
...
If this is "YES", all [engravings](Engravings.md) are displayed initially looking the same, like highlighted smooth walls/floors. Otherwise, they'll be displayed as a tile of one of the things they depict. Either way, a given engraving's behavior can be changed through designation.

- \[SHOW_FLOW_AMOUNTS:NO\]

If "YES" *Dwarf Fortress* displays water as numbers from 1-7 indicating [depth](Depth.md).
...
**Categories:** [DF2014:Exceptional Quality Articles](../Category/DF2014%253AExceptional_Quality_Articles.md) &middot; [DF2014:Game](../Category/DF2014%253AGame.md)

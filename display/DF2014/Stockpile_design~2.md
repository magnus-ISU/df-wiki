<!-- Mirrored from https://dwarffortresswiki.org/index.php/DF2014:Stockpile_design (revision 274572).
     Generated by crawl.py - edit the wiki, not this file. -->

# DF2014:Stockpile design

*[Source](https://dwarffortresswiki.org/index.php/DF2014:Stockpile_design) &middot; revision 274572 &middot; retrieved 2026-09-13 &middot; [wikitext](../../mirror/wikitext/DF2014/Stockpile_design~2.wiki)*

---
<table><tbody><tr><td><div><a href="../Main/Stockpile_design~2.md">v53.16</a> · <b>v0.47.05</b> · <a href="https://dwarffortresswiki.org/index.php/v0.34:Stockpile_design">v0.34</a> · <a href="https://dwarffortresswiki.org/index.php/v0.31:Stockpile_design">v0.31</a> · <a href="https://dwarffortresswiki.org/index.php/40d:Stockpile_design">v0.28</a> · <a href="https://dwarffortresswiki.org/index.php?title=23a:Stockpile_design&amp;action=edit&amp;redlink=1">v0.23</a></div>This article is about an <b>older version</b> of DF.<br/>
...
The classic example of this problem is the seed stockpile. Your planters need seeds to sow crops in farm plots. Seeds are stored in food stockpiles, in [bags](Bag.md); the bags in turn will be stored in [barrels](Barrel.md) or large pots if you allow it, and if enough empty barrels/pots are available. If you simply create a food stockpile next to the farm, and disable everything except seeds, the default settings will allow the use of barrels to store the seed bags. This will lead to job cancellations. Let's take a closer look at why this happens.
...
This design works well for inputs that come from a single location (e.g. plants from [farm plots](Farming.md)), and are used in many other locations. Rather than simply putting a plant stockpile by each workshop that uses plants (which would mean a great deal of hauling barrels from those workshops to the farm plots to pick up new plants), place one "source" plant stockpile by the farms which disallows barrels, and takes from anywhere. All the farm crops will go there first (directly, because no barrels are involved).

Now, place a plant stockpile by each workshop that uses plants ([still](Still.md), [farmer's workshop](Farmer's_workshop.md), etc.), set it to take from links only, and link it to take from the "source" stockpile. Ideally, each of these workshop-feeding stockpiles will take only an appropriately chosen subset of the plants (the still takes only brewable plants, while the farmer's workshop for the textile industry only takes plants that can be processed into [thread](Thread.md), and so on). You may choose whether to allow barrels on a case by case basis, depending on how many of each plant you expect to have at once, how time-critical their processing is, etc.

If you are so inclined, you can use a [quantum stockpile](Exploit.md#the-minecart-stop) as the "source". This allows an infinite amount of storage in a tiny space. In the specific case of plants, this means your harvest *always* has somewhere to go, and will never wilt on the ground (unless your haulers are all busy elsewhere).
...
One strategy is to place the feeder stockpile immediately around the workshop. This works well for single workshops with just one input (like a [mason's](Mason's_workshop.md) or [mechanic's workshop](Mechanic's_workshop.md)).
...
[Farms](Farming.md) feed a wide array of workshops. Plants from farms may directly feed the [kitchen](Kitchen.md), [still](Still.md), [millstone](Millstone.md) (or [quern](Quern.md)) and [farmer's workshop](Farmer's_workshop.md). The mill and farmer's workshop feed the [loom](Loom.md) and [dyer](Dyer.md) which feed the [clothier](Clothier.md). Farms are in turn supplied seeds from the still, mill, farmer's workshop and dining rooms, completing the cycle of plant life.

As such, it's a good idea to design all (or at least, many) of these workshops and their [stockpiles](Stockpile.md) around each other. Above-ground farms can only be built in certain places, and underground farms are much *easier* to build in soil layers, so in many cases this dictates the location and layout.

#### Butcher & Tanning

Because tanners are supplied directly from a butcher it's best to put them right next to each other. Designate a refuse stockpile taking Corpses for the butcher. Let the hides sit in the [butcher's shop](Butcher's_shop.md), and the tanner will take them directly to the [tanner's shop](Tanner's_shop.md) with no hauling jobs required.

Consider placing a [refuse stockpile](Stockpile.md#refuse) somewhere behind an airlock (to keep out the [miasma](Miasma.md)), to take the cartilage and other useless by-products.

- Note: When near a [necromancer tower](Necromancer.md), butcher shops and refuse piles where corpses are collected should be kept within the fortress out of line of sight of any necromancer. When in an evil [biome](Biome.md) which causes the dead to reanimate, corpses should be disposed of promptly, preferably in magma.

#### Kitchen

A food stockpile accepting meat/fish, raw edible [plants](Crop.md)\*†, [cheese](Cheese.md), [leaves](Quarry_bush.md), cookable [milled](Millstone.md) plants (everything but the [dyes](Dye.md)), [fat](Fat.md)†, cookable [extracts](Extract.md) ([Dwarven syrup](Dwarven_syrup.md) and all the [milks](Milk.md)†). If you cook [booze](Alcohol.md), include that. If you want to reserve some edible plants for [brewing](Still.md), leave them out. Allow the maximum barrels. Remember to turn off prepared food.

> *(\* [plump helmet](Plump_helmet.md), [muck root](Muck_root.md), [bloated tuber](Bloated_tuber.md), [prickle berry](Prickle_berry.md), [strawberry](Strawberry.md), [rat weed](Rat_weed.md), [fisher berry](Fisher_berry.md) and [sun berry](Sun_berry.md))*
> *(† These items generally have better uses than cooking: [milk](Milk.md) for [cheese](Cheese.md), [fat](Fat.md) for [tallow](Tallow.md) which is used in [soap](Soap.md) and [plants](Crop.md) for brewing as cooking will not return seeds. See [kitchen](Kitchen.md) for more information.)*

#### Brewing

A food stockpile accepting brewable [plants](Plant.md). Some of these have other uses, like pig tail for making thread, so you might want to disallow them if you plan on using them for something else. Remember to turn off prepared food.

Also include a barrel stockpile near your [still](Still.md) to ensure a steady supply of empty barrels.

### Use of wheelbarrows

[Wheelbarrows](Wheelbarrow.md) are tremendously useful tools, when properly applied. They allow a dwarf to move a single item at the dwarf's normal walking speed, *regardless of the item's weight*. But they have trade-offs, so they should not be used in every stockpile.
...
Wheelbarrows are used by default in stone stockpiles, and for good reason -- stones are *heavy*! Dwarves, especially [weak](Attribute.md#strength) ones, will move extremely slowly when hauling a stone without a wheelbarrow or [minecart](Minecart.md). It is strongly recommend that if you have any workshops which process stones, you use a feeder stockpile near them (linked to give to the workshop, so the mason doesn't carry a stone all the way from her bedroom!), and use wheelbarrows in that stockpile. This way, the stone is only ever carried by hand for a few tiles, from the stockpile to the workshop, by the mason/mechanic/craftsdwarf doing the work.
...
- [Quantum stockpiles](Exploit.md#quantum-stockpiles)

---

**Categories:** [DF2014:Exceptional Quality Articles](../Category/DF2014%253AExceptional_Quality_Articles.md) &middot; [DF2014:Design](../Category/DF2014%253ADesign.md) &middot; [DF2014:Guides](../Category/DF2014%253AGuides.md) &middot; [DF2014:Stockpiles](../Category/DF2014%253AStockpiles.md)

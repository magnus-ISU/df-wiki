<!-- Mirrored from https://dwarffortresswiki.org/index.php/Utility:SoundSense/Documentation (revision 231596).
     Generated by crawl.py - edit the wiki, not this file. -->

# Utility:SoundSense/Documentation

*[Source](https://dwarffortresswiki.org/index.php/Utility:SoundSense/Documentation) &middot; revision 231596 &middot; retrieved 2026-09-13 &middot; [wikitext](../../../mirror/wikitext/Utility/SoundSense/Documentation.wiki)*
...
SoundSense has a [bay12 forum thread](http://www.bay12forums.com/smf/index.php?topic=60287), an [official website](http://df.zweistein.cz/soundsense/), and is open source [here](https://code.google.com/p/df-soundsense/).

This page is for technical documentation, advanced instructions, advice for modders, and so on.

[Here's the main page for this utility](../SoundSense.md)

### Installation

SoundSense is included and preconfigured in [the Lazy Newb Pack](../Lazy_Newb_Pack.md) and requires no setup. For manual installation, continue...
...
- Install the java runtime 7. Get it [here](http://www.oracle.com/technetwork/java/javase/downloads/jre-7u3-download-1501631.html).
...
A few lines starting with \[java version "1.7.0_03"\] or something similar should appear. If that case is the case, you are good to go. But if you see \[java version "1.6.x_xx" or an earlier version (lower number), you will need to download and install the latest java.
...
Adventure Mode kind of works. The battle sounds function quite well because they are mostly the same as those in Fortress Mode. But SoundSense is still a bit lacking in other adventure sounds. The problem is that Adventure Mode does not log many other useful events. For example, encountering a megabeast or a quest mob is resolved in chat, leaving no message in the game log to trigger SoundSense.

Improvements in how DF handles logging could benefit both SoundSense and DF as a whole. Such suggestions have already been posted in the official Dwarf Fortress forums: [http://www.bay12forums.com/smf/index.php?topic=64834.0](http://www.bay12forums.com/smf/index.php?topic=64834.0)
...
File "soundSense.sh" has CR+LF characters at lines ending. Convert these characters to a more suitable form, with a tool like "[dos2unix](http://en.wikipedia.org/wiki/Unix2dos)":
...
Your Java virtual machine failed at reading MP3 files as audio stream. This could fix by adding to JVM an extra plugin: [JMF MP3 Plugin](http://www.oracle.com/technetwork/java/javase/download-137625.html). Download plugin, and put \*.jar files into your JVM "lib/ext/" directory.

##### Error at playing any MP3 file: "javax.sound.sampled.LineUnavailableException: Audio Device Unavailable"

Whereas your JVM succeeded in reading MP3 file as audio stream, it failed to access to an audio device. If your audio system uses [Alsa](http://fr.wikipedia.org/wiki/Advanced_Linux_Sound_Architecture), you could install "aoss" utility, and "soundSense.sh" will call it. On Debian, package "alsa-oss" provides this utility. **Note: if you are on Ubuntu see below**. Ubuntu uses PulseAudio backend, therefore toying with aoss is a waste of time.
...
First, **be sure that you are using OpenJDK 7**; you can't use OpenJDK 6 as it has quite a serious bug in its ALSA backend[\[1\]](http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6271108). You can check what java you have installed by typing
...
If you're using Sun/Oracle's JRE then switch to OpenJDK. If you don't want to do that (not recommended) then you might try to use the padsp wrapper or install osspd. If the padsp wrapper will not work properly for you then osspd daemon might just do the trick. (*[Archlinux Pulseaudio user](https://wiki.archlinux.org/index.php/Pulseaudio#OSS)*)

#### Any Other Issues? Post Feedback

There is a thread at the official Dwarf Fortress forums. Feel free to post any issues, suggestions, and contributions there: [http://www.bay12forums.com/smf/index.php?topic=60287.0](http://www.bay12forums.com/smf/index.php?topic=60287.0)
...
SoundSense supports .wav encoded in the "Windows PCM" format. However, some files with the .wav extension are actually encoded in some other proprietary format. A sound format conversion utility (such as FormatFactory) could be used to fix them.
...
- packSkeletons.zip ( [http://df.zweistein.cz/soundsense/packSkeletons.zip](http://df.zweistein.cz/soundsense/packSkeletons.zip) ) - This contains .xml files for all known gamelog lines currently without sounds. You can use this as a starting point to expand SoundSense.
...
- [http://www.musopen.org/](http://www.musopen.org/)
- [http://opengameart.org/browse/audio](http://opengameart.org/browse/audio)
- [http://www.incompetech.com/m/c/royalty-free/](http://www.incompetech.com/m/c/royalty-free/)
- [http://ccmixter.org](http://ccmixter.org)
- [http://free-loops.com/](http://free-loops.com/)
- [http://www.pdsounds.org/](http://www.pdsounds.org/)
- [http://commons.wikimedia.org/wiki/Category:Sound](http://commons.wikimedia.org/wiki/Category:Sound)
- [http://www.archive.org/details/opensource_audio](http://www.archive.org/details/opensource_audio)
- [http://publicdomainaudiovideo.blogspot.com/](http://publicdomainaudiovideo.blogspot.com/)
